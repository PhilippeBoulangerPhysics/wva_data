...
def open_experiment(exp_folder_name, start_file, end_file, file_name):
    ...
    ds = xar.open_mfdataset(files,  combine='nested', concat_dim='time',decode_times=True,parallel=True)  
    return ds
...
def get_pot_temp_np(temp,pfull,ps):
    k = 0.286
...
def plot_2d(diag,lat,lon,ax,cmap,level_space=25,land_mask = None,land_bool=False,proj = None,extend = "max"):
